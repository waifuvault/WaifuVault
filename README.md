...
| UPLOAD_RATE_LIMIT           | If set, this enables per-IP rate limiting on the `/rest` upload endpoint, independently of the global `RATE_LIMIT`. Defines how many uploads a single IP may make within the `UPLOAD_RATE_LIMIT_MS` window. Standard `RateLimit-*` headers are returned                                               |
| UPLOAD_RATE_LIMIT_MS        | If `UPLOAD_RATE_LIMIT` is set, this defines the length of the upload rate limit window in milliseconds. Defaults to `60000` (1 minute)                                                                                                                                                                |
