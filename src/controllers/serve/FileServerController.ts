...
export class FileServerController {
    ...
    public async getFile(
        ...
        try {
            await this.hasPassword(resource, req, password);
        } catch (e) {
    ...
    private async hasPassword(resource: string, req: Request, password?: string): Promise<boolean> {
        ...
        if (resourceIsProtected && !password) {
            if (req.path.includes("/f") && this.backendUrl === this.frontendUrl) {
